...
The NestJS API cannot run on Vercel (long-running server). Deploy to [Render](https://render.com) using `render.yaml` at the repo root.

After deploying the API, set these on Render:

| Variable | Value |
|----------|-------|
| `DATABASE_URL` | Supabase pooler URL + `?sslmode=require` |
| `DIRECT_URL` | Supabase direct/pooler URL + `?sslmode=require` |
| `JWT_SECRET` | Random string |
| `JWT_REFRESH_SECRET` | Random string |
| `CORS_ORIGIN` | `https://stitchhub-web.vercel.app` |

Then add `NEXT_PUBLIC_API_URL` on Vercel pointing to your Render API URL and redeploy.
